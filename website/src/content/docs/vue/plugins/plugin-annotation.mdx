...
const copyToAllPages = () => {
  ...
  if (selection) {
    const result = annotationApi.value?.duplicateAnnotationToAllPages(selection.object.id);
    if (result) {
...
annotationApi.value?.duplicateAnnotationToAllPages(annotationId, { includeSourcePage: true });
```
